(function (global, factory) {
  typeof exports === "object" && typeof module !== "undefined"
    ? factory(exports, require("d3-array"), require("d3-polygon"))
    : typeof define === "function" && define.amd
    ? define(["exports", "d3-array", "d3-polygon"], factory)
  ...
  HEdge.prototype.isEqual = function (origin, dest) {
    return (
      (this.orig.equals(origin) && this.dest.equals(dest)) ||
      (this.orig.equals(dest) && this.dest.equals(origin))
    );
  };
  ...
  (ConvexHull.prototype.prep = function () {
    ...
    for (var i = 2; i < this.points.length; i++) {
      if (
        !(
          linearDependent(v0, this.points[i]) &&
          linearDependent(v1, this.points[i])
        )
      ) {
        v2 = this.points[i];
        v2.index = 2;
  ...
  function computePowerDiagramIntegrated(
    sites,
    boundingSites,
    clippingPolygon
  ) {
      ...
      if (facet.isVisibleFromBelow()) {
        for (var e = 0; e < 3; e++) {
          ...
          if (!verticesVisited[destVertex.index]) {
            ...
            if (
              !site.isDummy &&
              d3Polygon.polygonLength(site.nonClippedPolygon) > 0
            ) {
              var clippedPoly = polygonClip(
                clippingPolygon,
                site.nonClippedPolygon
              );
  ...
  function weightedVoronoi() {
    ...
    function _weightedVoronoi(data) {
      ...
      return computePowerDiagramIntegrated(
        formatedSites,
        boundingSites(),
        clip
      );
