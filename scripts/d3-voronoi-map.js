(function (global, factory) {
  ...
  function FlickeringMitigation() {
    ...
    this.lastGrowth = NaN;
    this.growthChanges = [];
    this.growthChangeWeights = generateGrowthChangeWeights(
      this.growthChangesLength
    ); //used to make recent changes weighter than older changes
    this.growthChangeWeightsSum = computeGrowthChangeWeightsSum(
  ...
  FlickeringMitigation.prototype.length = function (_) {
    ...
    if (parseInt(_) > 0) {
      ...
      console.warn(
        "FlickeringMitigation.length() accepts only positive integers; unable to handle " +
          _
      );
    }
    return this;
  ...
  FlickeringMitigation.prototype.totalArea = function (_) {
    ...
    if (parseFloat(_) > 0) {
      ...
      console.warn(
        "FlickeringMitigation.totalArea() accepts only positive numbers; unable to handle " +
          _
      );
    }
    return this;
  ...
  function pie() {
    ...
    function updateInternals() {
      ...
      halfIncircleRadius =
        computeMinDistFromEdges(clippingPolygonCentroid, clippingPolygon) / 2;
  ...
  function voronoiMapSimulation(data) {
    ...
    function createMapPoints(basePoints, simulation) {
      ...
      return basePoints.map(function (bp, i, bps) {
        ...
        if (
          !d3Polygon.polygonContains(weightedVoronoi.clip(), initialPosition)
        ) {
    ...
    function adaptWeights(polygons, flickeringMitigationRatio) {
      ...
      for (var i = 0; i < siteCount; i++) {
        ...
        ); // in [(1-flickeringInfluence), 1]
        adaptRatio = Math.min(
          adaptRatio,
          1 + flickeringInfluence - flickeringMitigation
        ); // in [1, (1+flickeringInfluence)]
        //end: handle excessive change;
